...
from spider_dataset import get_spider_testset
...
def main(model_name: Optional[str] = None, batch_size: int = 1, quantize: bool = False):
    ...
    for c in ["EASY", "NON-NESTED", "NESTED"]:
        dataset = get_spider_testset(tokenizer, c)

        dataset.encodings.to(device)

        gold = open(f"./gold_query-test-{c}.txt", "w")
        generated = open(f"./generated-test-{c}.txt", "w")
